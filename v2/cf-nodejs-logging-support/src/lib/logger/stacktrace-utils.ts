export class StacktraceUtils {
    ...
    private static readonly MAX_STACKTRACE_SIZE = 55 * 1024;

    // check if the given object is an Error with stacktrace using duck typing
    static isErrorWithStacktrace(obj: any): boolean {
        ...
        if (totalLineLength > StacktraceUtils.MAX_STACKTRACE_SIZE) {
            ...
            for (let i = 0; i < fullStacktrace.length; i++) {
                if (i % 3 == 0) {
                    ...
                    if (currentLength + line.length > StacktraceUtils.MAX_STACKTRACE_SIZE) {
                        break;
                    }
